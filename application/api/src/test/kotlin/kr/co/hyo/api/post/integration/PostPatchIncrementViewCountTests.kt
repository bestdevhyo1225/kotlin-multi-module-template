...
@DisplayName("PATCH /posts/{id}/increment/view-count 테스트")
class PostPatchIncrementViewCountTests {
    ...
    fun `게시글의 조회수를 증가시키고, 상태코드 200을 응답한다`() {
        // given
        ...
        mockMvc
            .perform(
                patch("/posts/1/increment/view-count")
                    .contentType(APPLICATION_JSON)
                    .content(jacksonObjectMapper().writeValueAsString(postCreateRequest))
            )
